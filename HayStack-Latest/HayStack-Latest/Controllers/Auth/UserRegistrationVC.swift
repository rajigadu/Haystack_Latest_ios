...
class UserRegistrationVC: UIViewController {
    ...
    @IBOutlet weak var companeyNameTfref: UITextField!
...
//    }
    ...
    func RegistrationValidation()-> ApiCheckValidation {
        //DodidTfref.text ?? "" == "" ||
        if fnametfref.text ?? "" == "" || lnameTfref.text ?? "" == "" ||  emailTfref.text ?? "" == "" || mobileTfref.text ?? "" == "" || PasswordTfref.text ?? "" == "" || ConfirmPasswordTfref.text ?? "" == "" || addressTfref.text ?? "" == "" {
...
//            return}
        ...
        guard let password = PasswordTfref.text else{
            indicator.hideActivityIndicator()
            return}
        guard let companeyName = companeyNameTfref.text else{
            indicator.hideActivityIndicator()
            return}
...
extension UserRegistrationVC: GMSAutocompleteViewControllerDelegate {
